...
<c:forEach begin="1" end="10" step="1" varStatus="status">
	...
	<%-- 실제 합산 적용 : 
		 EL 은 추출전문 저장식은 불가능
		 ${summ = summ + status.index} 이러한 EL 식은 불가능
		 EL 은 추출전문 저장(할당)식은 불가능 따라서 <c:set> 사용 
	--%>
	<c:set var="summ" value="${summ + status.index}" scope="page"/>
</c:forEach>
summ 변수의 최종 값 : ${summ}
<br />
<hr noshade="noshade"/>
