...
/**
 * 서블릿의 생애주기(Lifecycle)을 관장하는 세개의 메소드
 * init(), service(), destroy() 를 사용해보는 
 * 서블릿 클래스
 */
...
public class LifeCycleServlet extends HttpServlet {
	...
	// init(), service() 메소드의 작동 확인을 위해 작동 횟수를 지정한 맴버 변수
	private int count;
	/**
	 *  이 서블릿이 최초로 호출되었을 때
	...
	/**
	 * 이 서블릿이 서비스로부터 헤제될 때(unload)될 때
	 * (서버가 종료될 때)
	 * 웹 컨테이너(톰캣)에 의해 단 한번 호출되며
	 * 서블릿 서비스를 종료하는 메소드
	 */
	public void destroy() {
		System.out.println("destory() 메소드가 실행되었습니다.");
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 출력객체(out)을 응답객체(respense)로 부터 얻어냄.
