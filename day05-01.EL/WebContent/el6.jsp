...
<table>
	...
	<tr><!-- 책은 도끼다 빈즈 객체로 추가된 것 EL로 추출 -->
		<td>${requestScope.ax.sequence}</td>
		<td>${requestScope.ax.isbn}</td>
		<td>${requestScope.ax.title}</td>
		<td>${requestScope.ax.author}</td>
		<td>${requestScope.ax.company}</td>
		<td>${requestScope.ax.totalPage}</td>
		<td>${requestScope.ax.price}</td>
		<td>${requestScope.ax.quantity}</td>
	</tr>
</table>

<hr />

<h3>Map 객체 로 사용자정의 객체 값 추출</h3>
<pre>
빈즈객체의 경우
1. .(도트) 연산자로 값을 추출
 (1) Book 객체 중 ax 라는 이름으로request에 추가된 객체가 있다면
 (2) ===> 내장객체이름.추가된속성명.필드명으로 값을 추출
 (3) \${requestScope.ax.sequence} 드으로 접근하여 값을 추출
...
맵 객체의 경우
1. .(도트) 연산자로 값을 추출
 (1) books 객체가 books 라는 이름으로 request 에 추가되었다면
 (2) ==> 내장객체이름.맵객체가추가된속성이름.맵의 키값 으로 맵의 value 를 추출
 (3) \${requestScope.books.b001.sequence}
 (4) 도트연산자 사용으로 접근시 주의점
 	==> 맵의 키가 숫자로 된 값인 경우 도트 연산자로 전근 불가
 	     이런 경우는 브래킷 연산자만 사용 가능

2. [](브래킷) 연산자로 값을 추출
 (1) books 객체가 books 라는 이름으로 request 에 추가되었다면
 (2) ==> 내장객체이름.맵객체가추가된속성이름["맵의 키값"].필드명으로 맵의 value 를 추출
 (3) \${requestScope.books["b001"].title}
 
맵, 리스트 등의 컬렉션이 속성으로 추가된 경우 주의점
 (1) 맵, 리스트 자체가 1개의 속성으로 추가되었음을 주의
 (2) 추가된 속성을 추출할 때는 맵, 리스트 등의 컬렉션 객체 1개를 추출해야 함을 주의
 (3) 추출한 각 컬렉션 객체안에서 맵이라면 키값, 리스트라면 인덱스값을 조합하여
       각 컬렉션에 들어있는 객체를 재 추출해야함
 (4) 컬렉션에 추가된 객체가 사용자 정의 객체(빈즈) 타입의 인스턴스라면
       추출된 빈즈객체의 각 필드명으로 또 다시.(도트) 연산자를 사용하여 접근해야됨.
</pre>
<table>
	<tr>
		<th>책 번호</th>
	...
	<tr><!-- 책은 도끼다 빈즈 객체로 추가된 것 Map로 추출 -->
